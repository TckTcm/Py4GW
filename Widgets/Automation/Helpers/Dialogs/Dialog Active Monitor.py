...
import Py4GW
from Py4GWCoreLib import Routines, PyImGui, Map, Agent, Dialog

MODULE_NAME = "Dialog Debug Monitor"
...
def _draw_header_controls() -> None:
    PyImGui.text("Dialog Debug Monitor")
    _same_line(12.0)
    ...
    if PyImGui.button("Export Raw"):
        ...
        Dialog.export_raw_callbacks_json(path, limit=20000, sync=True)
    _same_line(6.0)
    if PyImGui.button("Export Journal"):
        os.makedirs(_EXPORT_DIR, exist_ok=True)
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        path = os.path.join(_EXPORT_DIR, f"dialog_journal_{timestamp}.json")
        Dialog.export_callback_journal_json(
            path,
    ...
    if PyImGui.button("Export Turns"):
        ...
        Dialog.export_dialog_turns_json(
            path,
...
def _draw_current_state_panel(active, last_selected_id: int, dialog_active: bool) -> None:
    ...
    PyImGui.text_wrapped(active.message if active.message else "<empty>")
    if PyImGui.button("Copy Message"):
        PyImGui.set_clipboard_text(active.message if active.message else "")


def _draw_history_panel() -> None:
    PyImGui.text("History (Turns)")
    PyImGui.separator()
...
def _draw_raw_logs_panel() -> None:
    ...
    log_rows = Dialog.get_persisted_raw_callbacks(
        direction=direction,
...
def _draw_callback_journal_panel() -> None:
    ...
    _same_line(8.0)
    if PyImGui.button("Clear Journal"):
        if _dialog_api is not None:
    ...
    events = Dialog.get_persisted_callback_journal(
        direction=direction,
        npc_uid=_state.selected_npc_uid,
...
def _draw_ledger_panel() -> None:
    PyImGui.text("Dialog Turns (Core)")
    PyImGui.separator()

    all_turns = Dialog.get_dialog_turns(limit=400, include_choices=False, sync=False)
    ...
    _state.selected_npc_uid = npc_uids[new_index]
    ...
    if PyImGui.begin_child("TurnsPanel", (0, 230), True, PyImGui.WindowFlags.NoFlag):
        ...
        for turn in turns:
            ...
            if PyImGui.small_button(f"CopyTurnJSON##{turn_id}"):
                PyImGui.set_clipboard_text(json.dumps(turn, ensure_ascii=False))
            _same_line()
            if PyImGui.small_button(f"SelectTurn##{turn_id}"):
                _state.selected_turn_id = turn_id
            if body_text:
                PyImGui.text_wrapped(f"body={body_text}")
        if not turns:
            PyImGui.text("<no turns for selected npc>")
    PyImGui.end_child()

    if _state.selected_turn_id:
        selected_turn = Dialog.get_dialog_turn(_state.selected_turn_id, include_choices=True, sync=False)
        if selected_turn:
            PyImGui.separator()
            PyImGui.text(f"Selected Turn: {_state.selected_turn_id}")
            body_text = str(selected_turn.get("body_text_raw", "") or "")
            if body_text:
                PyImGui.text_wrapped(body_text)
            ...
            if choice_rows:
                ...
                for choice in choice_rows:
                    choice_id = int(choice.get("choice_dialog_id", 0) or 0)
                    choice_text = str(choice.get("choice_text_raw", "") or "")
...
def _draw_diagnostics_panel(active, choices) -> None:
    ...
    payload = Dialog.get_dialog_diagnostics(
    ...
    if PyImGui.begin_child("DiagnosticsPanel", (0, 180), True, PyImGui.WindowFlags.NoFlag):
        for index, issue in enumerate(diagnostics):
            ...
            line = f"[{severity}] {rule}: {message}{context_suffix}"
            PyImGui.text_wrapped(line)
            _same_line()
            if PyImGui.small_button(f"CopyJSON##diag_{index}"):
                PyImGui.set_clipboard_text(json.dumps(issue, ensure_ascii=False))
    PyImGui.end_child()
...
def draw_widget() -> None:
    ...
    if PyImGui.begin(MODULE_NAME):
        ...
        if PyImGui.begin_table("DialogMonitorLayout", 2, PyImGui.TableFlags.Resizable):
            PyImGui.table_setup_column("Left", PyImGui.TableColumnFlags.WidthFixed, 360)
            PyImGui.table_setup_column("Right", PyImGui.TableColumnFlags.WidthStretch, 0)
        ...
        _draw_ledger_panel()
        PyImGui.separator()
        _draw_diagnostics_panel(_state.current_active, _state.current_choices)
