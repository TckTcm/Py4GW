...
MODULE_BUILD = "send-once-guard-v55"
...
_sent_interact_agent_ids: set[int] = set()
...
def _forget_learned_mappings() -> None:
    ...
    _pending_click_agent_id = 0
    _pending_click_started_at = 0.0
    _sent_interact_agent_ids.clear()
...
def _process_pending_click_timeout() -> None:
    ...
    if not _pending_click_agent_id:
        return
    now = time.monotonic()
    if now - _pending_click_started_at < _INTERACT_CONFIRM_TIMEOUT:
        return

    agent_id = _pending_click_agent_id
    if _advance_click_plan_for_agent(
        agent_id,
        f"No CToS/server confirmation for switch agent {agent_id}; assuming interaction was queued",
    ):
    ...
    _last_click_time = now
    _set_status(
        f"No switch confirmation for agent {agent_id}; waiting before retry.",
        Py4GW.Console.MessageType.Warning,
    )
...
def _click_next_switch() -> None:
    ...
    now = time.monotonic()
    if agent_id in _sent_interact_agent_ids:
        _last_click_time = now
        if _advance_click_plan_for_agent(
            agent_id,
            f"Switch agent {agent_id} was already interacted with; suppressing duplicate click",
        ):
            return
        _set_status(
            f"Duplicate click suppressed for switch agent {agent_id}.",
            Py4GW.Console.MessageType.Warning,
        )
        return
    ...
    _sent_interact_agent_ids.add(agent_id)
    _last_click_time = now
    _last_move_agent_id = 0
    _last_move_time = 0.0
